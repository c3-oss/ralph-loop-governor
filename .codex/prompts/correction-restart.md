...
Use this executor command shape. Keep the natural-language prompt quoted:
...
Before restart, make sure every blocking correction has an ID, severity, risk, required fix, acceptance criteria, and evidence fields. Update `status.md`, `gates.md`, and lane evidence so Ralph can resume from the first open blocker without reinterpreting the run. The correction prompt must also preserve the five-cycle final stabilization wait requirement.
